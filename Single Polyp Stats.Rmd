...
lmer(bc.dist~physical.dist + (1|Colony.id) + (1|Colony.id:Branch.id), data=Dist.compare.df )
...
Dist.mod<-lmer(bc.dist~physical.dist + (1|Coral) + (1|Coral:Branch), data=Dist.compare.df )
anova(Dist.mod)  #species differ
qqPlot(residuals(Dist.mod)) 
emm<-emmeans(Dist.mod,  ~ physical.dist, adjust="Tukey")
cld(emm)
pairs(emm)

mod<- lmer(bc.dist~PolypZones + (1|Coral) + (1|Coral:Branch) , data=Dist.compare.df )
anova(mod)
